...
{
		float amplitude=6; //Wheel target speed limit //车轮目标速度限幅原3.5
	  ...
	  if (Car_Mode==Mec_Car) 
    {
		...
		else if (Car_Mode==Akm_Car) 
		{
			...
			//舵机PWM值，舵机控制前轮转向角度
			Angle_Servo    =  -0.628f*pow(AngleR, 3) + 1.009f*pow(AngleR, 2) - 1.772f*AngleR + 1.573f;
			Servo=SERVO_INIT + (Angle_Servo*Ratio);
			float Turn_Assist = 0.2f; // 转向辅助系数，越大转弯越贼！(建议0.2~0.8)
			MOTOR_A.Target = MOTOR_A.Target - (Vz * Turn_Assist); // 左转时，左轮主动减速
			MOTOR_B.Target = MOTOR_B.Target + (Vz * Turn_Assist); // 左转时，右轮主动加速
			//Wheel (motor) target speed limit //车轮(电机)目标速度限幅
			MOTOR_A.Target=target_limit_float(MOTOR_A.Target,-amplitude,amplitude); 
			MOTOR_B.Target=target_limit_float(MOTOR_B.Target,-amplitude,amplitude); 
...
{ 	
	 ...
	 Pwm+=Velocity_KPL*(Bias-Last_bias)+Velocity_KIL*Bias; 
...
{  
	 static float Bias,Pwm,Last_bias;
	 Bias=Target-Encoder; //Calculate the deviation //计算偏差
	 Pwm+=Velocity_KPR*(Bias-Last_bias)+Velocity_KIR*Bias;  
	 if(Pwm>16700)Pwm=16700;
	 if(Pwm<-16700)Pwm=-16700;
	 Last_bias=Bias; //Save the last deviation //保存上一次偏差 
	 return Pwm;
...
void Get_Velocity_Form_Encoder(void)
{
	...
	//test_num=OriginalEncoder.B;
	  ...
	  //Decide the encoder numerical polarity according to different car models
		...
		// 在算完 MOTOR_A.Encoder = ... 之后加入：
		static float last_Encoder_A = 0;
		// 0.3 是滤波系数，可调。能让速度曲线平滑，电机声音更纯净
		MOTOR_A.Encoder = 0.3f * MOTOR_A.Encoder + 0.7f * last_Encoder_A; 
		last_Encoder_A = MOTOR_A.Encoder;
		static float last_Encoder_B = 0;
		MOTOR_B.Encoder = 0.3f * MOTOR_B.Encoder + 0.7f * last_Encoder_B;
		last_Encoder_B = MOTOR_B.Encoder;
		static float last_Encoder_C = 0;
		MOTOR_C.Encoder = 0.3f * MOTOR_C.Encoder + 0.7f * last_Encoder_C;
		last_Encoder_C = MOTOR_C.Encoder;
		static float last_Encoder_D = 0;
		MOTOR_D.Encoder = 0.3f * MOTOR_D.Encoder + 0.7f * last_Encoder_D;
		last_Encoder_D = MOTOR_D.Encoder;
}
/**************************************************************************
